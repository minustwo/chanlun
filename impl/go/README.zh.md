...
`conformance/chanlun-v1/reference_backend/*.py` 的忠实纯标准库 Go 移植；
在 SHA 等同律下，逐字节通过冻结的 `chanlun-v1` 语料库的全部 48 个 fixture。

## 这是什么

语料库 `conformance/chanlun-v1` 是缠论 pipeline 的冻结多语言一致性规范
（48 fixture × 6 stage，按字节锁定的 SHA-256；
`corpus_sha256 = df9c4f7ef0ca42bde51ed4db9ee0e4b13c8a11776e14324cfb6b9d32af7dd5c5`）。
任一语言的实现，当且仅当它在每个 fixture 的 `input` 上的分解结果在
...
* **整数语义**，与 Python 参考一致。若输入 fixture 出现浮点数，解码器
  直接拒绝，而不是静默强转。
* **严格六个 stage**，与语料库一致：`normalize`、`fractal`、`stroke`、
  `zhongshu`、`trend_type`、`pipeline`。超出语料库的 stage（segment、
  level-recursion、walk-decomposition）属于 Lean 库，但有意不在此处
  移植。
...
SHA 等同律所比对的字节，是 Python 用下面这种规范方式产生的：
...
1. **键名排序** —— 对象键按原始字符串键的 Unicode 码点序排序。
   chanlun-v1 语料库中所有键都是 ASCII，所以等价于字典序的字节序，正是
   Python 在 `str` 键上 `sort_keys=True` 的行为。
2. **零空白** —— `,` 与 `:` 两侧均无空格（对应
   `separators=(",", ":")`）。
3. **`ensure_ascii=True`** —— 每一个码点 >= 0x7f 都转义成 `\uXXXX`。
   超出 BMP 的码点使用 UTF-16 代理对（与 Python `json.dumps` 的实际行为
   完全一致）。标准短转义（`\"`、`\\`、`\b`、`\f`、`\n`、`\r`、`\t`）
   照常使用；其它控制字符（< 0x20）使用 `\u00XX` 输出。
...
规范字节完全一致 —— 全部 48 个 `expected_sha256` 重新计算都对，而
harness 对 `corpus_sha256` 的整体重算也复现了 manifest 中的
...
（通过 `\uXXXX` 带代理处理的显式路径）；非整数则需要对语料库契约做
显式扩展 —— 今天的契约不允许这种情况。

## 来源

`conformance/chanlun-v1/reference_backend/` 的 Python 参考是 stage
语义的唯一权威；本 Go 移植精确复现这套语义。本目录不发明任何后端
逻辑 —— 与 Python 的偏差仅限于规范 JSON 输出层；SHA 等同 gate 即是
检测漂移的回归测试。
...
非零退出码即 fail。一个字节不对就是 FAIL，绝不静默跳过。
